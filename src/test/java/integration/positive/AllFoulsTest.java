...
public class AllFoulsTest {
	
	@Test
	public void allFoulsExecutedSuccessfully() throws IOException {
		...
		List<Play> plays = parser.parse("src\\test\\resources\\positive\\fouls.txt");
		...
		boolean actual = FileUtils.contentEquals(new File("src\\test\\resources\\test_output.txt"),
				new File("src\\test\\resources\\filesToCompare\\correct_all_fouls_output.txt"));
		assertTrue(actual);
	}
}
